require 'nn'
--[[
    Taken from https://github.com/y0ast/VAE-Torch
]]--

local GaussianCriterion, parent = torch.class('nn.GaussianCriterion', 'nn.Criterion')

function GaussianCriterion:__init(scale)
...
function GaussianCriterion:updateOutput(input, target)
    ...
    Gelement:add(torch.add(target,-1,input[1]):pow(2):cdiv(torch.exp(input[2]) + 1e-10):mul(0.5))
